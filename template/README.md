...
| `.claude/skills/` | Six Claude Code skills: `/sdd-feature` (the feature loop), `/landmine-check` (pre-commit silent-failure sweep), `/tstl-lua-gotchas` (where compiled Lua diverges from TS), `/ability-modifier-patterns`, `/kv-authoring`, `/panorama-ui` |
